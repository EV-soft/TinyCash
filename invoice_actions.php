...
function clean($conn, $data) {
    return DB::real_escape_string($conn, trim($data));
}
...
switch ($action) {
    ...
    case 'create_product':
        ...
        if (DB::query($conn, $sql)) {
            ...
            die(lang("@Error creating product: ") . DB::error($conn));
        }
        break;
    ...
    case 'update_product':
        ...
        if (DB::query($conn, $sql)) {
            ...
            die("Error updating product: " . DB::error($conn));
        }
        break;
    ...
    case 'delete_product':
        ...
        $check = DB::query($conn, "SELECT line_id FROM invoice_lines WHERE prod_id = $id LIMIT 1");
        if (DB::num_rows($check) > 0) {
            header("Location: inventory_status.php?msg=error_in_use");
        } else {
            DB::query($conn, "DELETE FROM products WHERE prod_id = $id");
    ...
    case 'slet_faktura':
        ...
        $res_lines = DB::query($conn, "SELECT prod_id, quantity FROM invoice_lines WHERE inv_id = $inv_id");
        if ($res_lines) {
            while ($line = DB::fetch_assoc($res_lines)) {
                $p_id = (int)$line['prod_id'];
                $qty  = (float)$line['quantity'];
                if ($p_id > 0) {
                    DB::query($conn, "UPDATE products SET prod_stock = prod_stock + $qty WHERE prod_id = $p_id");
                }
            }
        }
        // 2. Slet data (linjer først pga. relationer)
        DB::query($conn, "DELETE FROM invoice_lines WHERE inv_id = $inv_id");
        if (DB::query($conn, "DELETE FROM invoices WHERE inv_id = $inv_id")) {
    ...
    case 'update_status':
        ...
        DB::query($conn, "UPDATE invoices SET inv_status = '$status' WHERE inv_id = $id");
