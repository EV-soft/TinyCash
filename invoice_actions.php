<?php # /invoice_actions.php v:0.8.0.1 d:2026-04-10 i:Gemini m:1
...
switch ($action) {
    ...
    case 'create_product':
        ...
        if (mysqli_query($conn, $sql)) {
            header("Location: inventory_status.page.php?msg=created");
        } else {
            die("Error creating product: " . mysqli_error($conn));
        }
        break;
    ...
    case 'update_product':
        ...
        if (mysqli_query($conn, $sql)) {
            header("Location: inventory_status.page.php?msg=updated");
        } else {
    ...
    case 'delete_product':
        ...
        if (mysqli_num_rows($check) > 0) {
            header("Location: inventory_status.page.php?msg=error_in_use");
        } else {
            mysqli_query($conn, "DELETE FROM products WHERE prod_id = $id");
            header("Location: inventory_status.page.php?msg=deleted");
        }
        break;
    ...
    case 'slet_faktura':
        ...
        if (mysqli_query($conn, "DELETE FROM invoices WHERE inv_id = $inv_id")) {
            header("Location: invoices.page.php?msg=slettet");
        }
        break;
    ...
    case 'update_status':
        ...
        header("Location: invoices.page.php?msg=status_opdateret");
        break;
