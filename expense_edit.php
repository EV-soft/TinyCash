<?php # /expense_edit.php v:1.1.0 d:2026-07-02 i:evs
ob_start();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($_FILES['attachment']['name']) && $_FILES['attachment']['error'] == 0) {
        ...
        $absolute_depot_path = file_exists(__DIR__ . '/../' . $depot_source_path) ? __DIR__ . '/../' . $depot_source_path : __DIR__ . '/' . $depot_source_path;
    ...
    // print_arr('_POST'); 
    // 1. Indlæs eksisterende værdier sendt fra formularen først
    ...
    // 2. Kør OpenAI én gang og overskriv værdierne direkte
    if ($use_ai_scan === 1 && $file_to_scan && file_exists($file_to_scan)) {
        $ai_data = scanBilagMedOpenAI($file_to_scan);
        if (isset($ai_data['error'])) {
            echo "<pre>OpenAI Error: "; print_r($ai_data['error']); echo "</pre>"; die();
        }
        if ($ai_data) {
            // Hvis der blev trykket på "Hent data med AI" ELLER hvis felterne reelt er tomme/standard
            $is_scan_action = isset($_POST['action_scan_only']);
            if ($is_scan_action || empty($_POST['exp_date']) || $_POST['exp_date'] === date('Y-m-d')) {
                if (!empty($ai_data['dato'])) {
                    $date = $ai_data['dato'];
                    $_POST['exp_date'] = $date; // Tving værdien ind i POST, så htm_InputGroup fanger den
                }
            }
            if ($is_scan_action || empty($_POST['amount']) || $_POST['amount'] === '0,00' || $_POST['amount'] === '0') {
                if (isset($ai_data['total'])) {
                    // OpenAI returnerer typisk punktum (280.21). Vi gemmer det som rå streng eller float.
                    $amount = (string)$ai_data['total'];
                    $_POST['amount'] = $amount; // Tving værdien ind i POST
                }
            }
            if ($is_scan_action || empty($_POST['supplier'])) {
                if (!empty($ai_data['leverandor'])) {
                    $supp = $ai_data['leverandor'];
                    $_POST['supplier'] = $supp; // Tving værdien ind i POST
                }
            }
            if ($new_filename) { 
                $current_attachment = $new_filename; 
                $new_filename = null; 
            }
        }
    }
    ...
    if (isset($_POST['action_save'])) {
        ...
        if ($id > 0) {
            $sql = "UPDATE expenses SET exp_date = '$db_date', supplier = '$db_supp', description = '$db_desc', account_id = '$db_acc', amount = '$db_amount' $attachment_sq WHERE exp_id = $id AND is_cancelled = 0";
            $final_voucher = $vouch;
        } else {
            ...
            $sql = "INSERT INTO expenses (exp_date, supplier, description, account_id, amount, attachment, voucher_no, created_by) VALUES ('$db_date', '$db_supp', '$db_desc', '$db_acc', '$db_amount', '$final_file', $final_voucher, $current_user_id)";
        }
        ...
        if (DB::query($conn, $sql)) {
            ...
            if ($jou_row = DB::fetch_assoc($check_jou)) {
                DB::query($conn, "UPDATE journal SET jou_date = '$db_date', voucher_no = '$final_voucher', jou_text = 'Udgift ID $expense_id: $journal_text' WHERE jou_id = ".$jou_row['jou_id']);
            } else {
                DB::query($conn, "INSERT INTO journal (jou_date, voucher_no, jou_text) VALUES ('$db_date', '$final_voucher', 'Udgift ID $expense_id: $journal_text')");
            }
...
htm_Header($id > 0 ? lang('@Edit Expense') : lang('@Add Expense'));
showMenu();
htm_HelpSystem(); 
...
echo '<div style="display:flex; gap:10px; max-width:1400px; margin:0 auto; padding:0 10px; align-items: flex-start;">';
    ...
    // VENSTRE SIDE (Dropzone og Bilagsvisning)
    echo '<div style="flex: 6; min-width: 300px; position: sticky; top: 20px;">';
        htm_Card_('🗄️ ' . lang('@Depot & Dropzone'), '100%', '', false);
          echo '<div id="MainDropzone" style="margin-bottom:15px; border:2px dashed #34495e; border-radius:4px; height:calc(100vh - 510px); min-height:350px; background:#f1f2f6; position:relative; overflow:hidden !important; user-select:none;">';
                ...
                // 1. Vejledning i den tomme placeholder (Oversat via lang)
                ...
                // 2. Hjælpe-ikon med tooltip (Oversat via lang)
    ...
    // HØJRE SIDE (Original struktur)
    echo '<div style="flex: 6; min-width: 320px;">';
        htm_Card_('📝 ' . ($id > 0 ? lang('@Edit Expense') : lang('@Register expense')), '100%');
            echo $header_switch = '<label style="font-weight:bold; cursor:help; display:flex; align-items:center; gap:6px; 
            font-size:14px; user-select:none; color:#7f8c8d; margin:0;"><input type="checkbox" id="visible_ai_trigger" '.
            ($use_ai_scan === 1 ? 'checked' : '').' onchange="triggerAiToggle(this.checked);" 
            style="width:14px; height:14px; cursor:pointer; margin:0; vertical-align:middle;">🤖 ' . 
            lang('@AI Scan') . ': ' . 
            ($use_ai_scan === 1 ? '<span style="color:#2ecc71;">' . lang('@Active'). 
            '<span style="color:#000; font-weight:normal; ">  -- '. lang('@Colored fields can be filled by AI Scan.</span>') . '</span>' 
            : '<span style="color:#e67e22;">' . lang('@Off') . '</span>') . '</label><br>'; 
            
            echo '<div style="padding:5px; margin-bottom:15px; font-size:12px; color:#4a5568; line-height:1.4;">';
            ...
            htm_InputGroup('fa-coins', '@Amount', 'amount', number_format((float)$amount, 2, ',', ''), 'text', null, 'required style="text-align: right;"', '30%'); 
            ...
            // PLACERET EFTER OVERSKRIFTEN HER:
            echo '<div style="margin-bottom:15px; padding:0 5px; margin-top:15px;">';
            echo '<label style="display:block; font-weight:bold; margin-bottom:8px; font-size:13px; color:#2c3e50; text-align:left;">';
            echo '<i class="fa fa-list" style="margin-right:5px; color:#7f8c8d;"></i> ' . lang('@Select Finance Account');
            echo '<span id="AccountPanelHint" style="font-size:11px; color:#e67e22; font-style:italic; font-weight:normal; margin-left:10px;">';
            echo '(<i class="fa-solid fa-lock"></i> ' . lang('@Please select or upload a voucher first to unlock account selection.') . ')';
            echo '</span>';
            echo '</label>';
            echo '<div id="QuickCategoryPanel" style="display:flex; flex-wrap:wrap; gap:8px; opacity:0.3; pointer-events:none; transition:opacity 0.3s;">';
                ...
                while($account = DB::fetch_assoc($a_res)) {
                    $acc_id = $account['acc_id']; $full_name = $acc_id . " - " . $account['acc_name'];
                    echo '<label class="radio-card-btn '.($acc == $acc_id ? 'active-radio-btn' : '').'" title="'.htmlspecialchars($full_name).'"><input type="radio" name="account_id" value="'.$acc_id.'" '.($acc == $acc_id ? 'checked' : '').' onchange="highlightSelectedRadio(this)" required><span>'.$full_name.'</span></label>';
                }
            echo '</div></div>';

            if (!empty($current_attachment)) {
                echo '<div style="margin-bottom:15px; padding:10px; background:#f8f9fa; border-left:4px solid #e67e22; display:flex; justify-content:space-between; align-items:center; border-radius:4px;"><div>' . htm_GetDocIcon($current_attachment, 'expense') . ' <a href="uploads/'.$current_attachment.'" target="_blank" style="font-weight:bold; text-decoration:none;">' . lang('@View current attachment') . '</a><br><span style="font-size:11px; color:#7f8c8d;">' . lang('@Filename') . ': '.$current_attachment.'</span></div><label style="color:#c0392b; font-size:13px; cursor:pointer;"><input type="checkbox" name="delete_attachment" value="1" style="margin-right:5px; vertical-align:middle;"> ' . lang('@Remove attachment') . '</label></div>';
            }
            ...
            echo '<div style="display:flex; gap:10px; margin-top:20px; border-top:1px solid #eee; padding-top:20px;"><button type="submit" name="action_save" value="1" style="flex:2; background:#2ecc71; color:white; border:none; padding:10px; border-radius:4px; font-weight:bold; cursor:pointer;"><i class="fa fa-save"></i> ' . lang('@Save Expense') . '</button>';
            htm_Button('fa-times', lang('@Cancel'), 'secondary', 'expense_list.php', 'flex:1;'); echo '</div>';
        htm_Card_end();
    echo '</div>';
echo '</div>';
echo '</form>';
...
function triggerAiToggle(c) { 
    document.getElementById('hidden_ai_checkbox').checked = c; 
    document.getElementById('hidden_ai_action').value = "1"; 
    document.getElementById('global_exp_form').submit(); 
}

function tjekVisScanKnap() { 
    scanBtn.style.display = (<?php echo $use_ai_scan; ?> === 1 && (MidlertidigFilFraPC || document.getElementById('depot_file_path').value !== '')) ? 'block' : 'none'; 
}
...
function nulstilVisning() { 
    ...
    if(document.getElementById('AccountPanelHint')) document.getElementById('AccountPanelHint').style.display = 'inline';
    
    const dzRect = dropzone.getBoundingClientRect();
    zoomWrapper.style.height = (dzRect.width * 1.414) + "px";
    
    if (pdfPreview.style.display === 'block') {
        zoomWrapper.style.width = (dzRect.width * 1.04) + "px";
    } else {
        zoomWrapper.style.width = dzRect.width + "px";
    }
    
    opdaterTransform();
}

function previewDepotFile(p) { 
    nulstilVisning(); 
    const e = p.split('.').pop().toLowerCase(); 
    if (e === 'pdf') { 
        pdfPreview.src = p + "#toolbar=0&navpanes=0"; 
    ...
    if(document.getElementById('AccountPanelHint')) document.getElementById('AccountPanelHint').style.display = 'none';
}

function selectDepotFile(e, p, f) { 
    e.stopPropagation(); 
    MidlertidigFilFraPC = null; 
    document.getElementById('depot_file_path').value = p; 
    document.getElementsByName('attachment')[0].value = ""; 
    badge.innerText = '✓ ' + "<?php echo lang('@Depot'); ?>" + ': ' + f; 
...
function opdaterTransform() {
    const dzRect = dropzone.getBoundingClientRect();
    const reelleHojde = parseFloat(zoomWrapper.style.height) || dzRect.height;
    
    const transformeretBredde = dzRect.width * currentZoom;
    const transformeretHojde = reelleHojde * currentZoom;

    const maxPanX = 0;
    const minPanX = dzRect.width - transformeretBredde;
    translateX = (transformeretBredde <= dzRect.width) ? 0 : Math.min(maxPanX, Math.max(minPanX, translateX));

    const maxPanY = 0;
    const minPanY = dzRect.height - transformeretHojde;
    
    if (transformeretHojde <= dzRect.height) {
        translateY = 0;
    } else {
        translateY = Math.min(maxPanY, Math.max(minPanY, translateY));
    }
...
dropzone.addEventListener('mousedown', (e) => {
    const dzRect = dropzone.getBoundingClientRect();
    const reelleHojde = parseFloat(zoomWrapper.style.height) || dzRect.height;
    if (currentZoom === 1 && (reelleHojde <= dzRect.height)) return;
...
dropzone.addEventListener('wheel', (e) => {
    ...
    const nextZoom = Math.min(Math.max(1, currentZoom + delta * zoomIntensity), 5);
    ...
    const docX = (mouseX - translateX) / currentZoom;
    const docY = (mouseY - translateY) / currentZoom;

    currentZoom = nextZoom;
    translateX = mouseX - docX * currentZoom;
    translateY = mouseY - docY * currentZoom;
...
document.addEventListener("DOMContentLoaded", () => {
    skiftDepotTab('mail');
    const f = '<?php echo $current_attachment; ?>';
    if(f !== '') { 
        previewDepotFile('uploads/' + f); 
        badge.innerText = '✓ ' + "<?php echo lang('@Voucher ready'); ?>" + ': ' + f; 
        badge.style.background = '#2ecc71'; 
        badge.style.display = 'block'; 
    }
    
    if (document.getElementById('depot_file_path').value !== '' || f !== '' || MidlertidigFilFraPC !== null) { 
        panel.style.opacity = '1'; 
        panel.style.pointerEvents = 'auto'; 
        if(document.getElementById('AccountPanelHint')) document.getElementById('AccountPanelHint').style.display = 'none';
...
function highlightSelectedRadio(r) {
    document.querySelectorAll('.radio-card-btn').forEach(l => l.classList.remove('active-radio-btn'));
    if (r.checked) {
        r.closest('.radio-card-btn').classList.add('active-radio-btn');
        if (MidlertidigFilFraPC || document.getElementById('depot_file_path').value !== '' || '<?php echo $current_attachment; ?>' !== '') { 
            badge.innerText = '📂 ' + "<?php echo lang('@Voucher to account'); ?>" + ' #' + r.value; 
            badge.style.background = '#2ecc71'; 
        }
    }
}

function handleLocalFileSelect(i) {
    if (i.files && i.files[0]) {
        MidlertidigFilFraPC = i.files[0]; 
        document.getElementById('depot_file_path').value = ''; 
        
        if (MidlertidigFilFraPC.type === "application/pdf") {
            pdfPreview.style.display = 'block';
        } else {
            imgPreview.style.display = 'block';
        }
        ...
        nulstilVisning();
        const u = URL.createObjectURL(MidlertidigFilFraPC);
        if (MidlertidigFilFraPC.type === "application/pdf") { pdfPreview.src = u; }
        else if (MidlertidigFilFraPC.type.startsWith("image/")) { imgPreview.src = u; }
        
        badge.innerText = '📂 ' + "<?php echo lang('@Local file'); ?>" + ': ' + MidlertidigFilFraPC.name; 
...
dropzone.addEventListener('drop', (e) => {
    e.preventDefault(); dropzone.style.background = '#f1f2f6'; dropzone.style.borderColor = '#34495e';
    if (e.dataTransfer.files.length > 0) { 
        MidlertidigFilFraPC = e.dataTransfer.files[0]; 
        const inp = document.getElementsByName('attachment')[0], dt = new DataTransfer(); 
        dt.items.add(MidlertidigFilFraPC); 
        inp.files = dt.files; 
        handleLocalFileSelect(inp); 
    }
});
</script>
...
<?php
// ENKELT FAST PLACERET SYSTEMHJÆLP-KNAP
$current_page = basename($_SERVER['SCRIPT_NAME']);
$target_lang  = isset($_SESSION['lang']) ? $_SESSION['lang'] : 'da';

echo '
<script>
function openHelpSystem(page) {
    var modal = document.getElementById("help-modal-container");
    if (modal) {
        modal.style.display = "block";
        if (typeof initHelpDrag === "function") {
            initHelpDrag();
        }
    } else {
        console.warn("Hjælpesystem: #help-modal-container manglede. Henter indhold...");
        alert("' . lang('@Help') . ': ' . lang('@No help text for this page') . '");
    }
}
</script>';

echo '<div class="page-footer-help-trigger" style="position: fixed; bottom: 15px; right: 15px; z-index: 9999;">';
echo '  <button type="button" class="btn btn-secondary btn-sm" onclick="openHelpSystem(\''.$current_page.'\')" style="background: #e67e22; color: white; border: none; padding: 6px 12px; border-radius: 4px; font-weight: bold; cursor: pointer;"><i class="fa fa-question-circle"></i> ' . lang('@Help') . '</button>';
echo '</div>';
?>