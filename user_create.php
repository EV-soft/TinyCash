...
<form method="post">
    <?php
    htm_InputGroup('', lang('@Username'), 'username', '', 'text');
    $role_options = ['user' => lang('@User'), 'admin' => lang('@Administrator')];
    htm_InputGroup('', lang('@Role'), 'user_role', 'user', 'select', $role_options);
    echo "<hr style='margin:25px 0; border:0; border-top:1px dashed #ccc;'>";
    htm_InputGroup('', lang('@Password'), 'password', '', 'password');
    htm_InputGroup('', lang('@Confirm Password'), 'confirm_password', '', 'password');
    ?>
